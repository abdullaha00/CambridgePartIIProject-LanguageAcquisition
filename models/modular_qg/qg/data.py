...
class QGDataset:
    ...
    def __init__(
        ...
        num_state_samples_per_user: int = 5,
        num_questions_per_state: int = 5,
        ...
        for _uid, hist in tqdm(histories.items(), desc="Building QG examples", leave=False):
            if len(hist) < min_prefix_len:
                logger.warning(f"Skipping user {_uid} with history length {len(hist)} < min_prefix_len {min_prefix_len}")
                continue
            ...
            for _ in range(num_state_samples_per_user):
                ...
                # we sample random points in the learner history
                prefix_len = np.random.randint(min_prefix_len, max_prefix_len + 1)
                pref_text = history_text(hist[:prefix_len])

                # and select sample questions to insert at that point
                sampled_qs = np.random.choice(held_out_qs, size=n_sample, replace=False)
