...
for lang in tqdm(["en_es", "es_en", "fr_en"], desc="Language"):
    ...
    os.makedirs(f"parquet/{lang}", exist_ok=True)
    ...
    for typ in tqdm(["train", "dev", "test"], desc=f"{lang} split", leave=False):
        ...
        dfM.to_parquet(f"parquet/{lang}/{typ}/{lang}_{typ}_minimal.parquet", index=False)
        df.to_parquet(f"parquet/{lang}/{typ}/{lang}_{typ}_original.parquet", index=False)
